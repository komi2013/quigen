(function() {
    ...
    ImgTouchCanvas.prototype = {
        ...
        doZoom: function(zoom) {
            ...
            var newScale = this.scale.x + zoom/1000;
        ...
        doMove: function(relativeX, relativeY) {
            if(this.lastX && this.lastY) {
              ...
              var currentWidth = (this.imgTexture.width * this.scale.x);
              var currentHeight = (this.imgTexture.height * this.scale.y);
              if(gesturableImg.rotate % 4 == 1){
                this.position.x += deltaY;
                this.position.y += -deltaX;
              }else if(gesturableImg.rotate % 4 == 2){
                this.position.x += -deltaX;
                this.position.y += -deltaY;
              }else if(gesturableImg.rotate % 4 == 3){
                this.position.x += -deltaY;
                this.position.y += deltaX;
              }else if(gesturableImg.rotate % 4 == 0){
                this.position.x += deltaX;
                this.position.y += deltaY;
              }
        ...
        setEventListeners: function() {
            ...
            if(this.desktop) {
                window.addEventListener('keyup', function(e) {
                    if(e.keyCode == 187 || e.keyCode == 61 || e.keyCode == 59) { //+
                        this.doZoom(5);
                    }
                    else if(e.keyCode == 54 || e.keyCode == 173) {//-
                        this.doZoom(-5);
                    }
                }.bind(this));
                ...
                window.addEventListener('mousemove', function(e) {
                    ...
                    if(relativeX <= 0 || relativeX -300 >= this.canvas.clientWidth || relativeY <= 0 || relativeY -300 >= this.canvas.clientHeight) {
                        this.mdown = false;
                    }
                }.bind(this));
            }
            ...
            minus.addEventListener('click', function(e) {
              this.doZoom(localStorage.scale * -1);
            }.bind(this));
            var plus = document.getElementById('plus');
            plus.addEventListener('click', function(e) {
              this.doZoom(localStorage.scale);
            }.bind(this));
        },
